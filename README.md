...
scikit-learn models (#3), gradient boosting (#4) and SHAP explanations (#5).
...
[results/shap_summary_xgboost_CA_1.png](results/shap_summary_xgboost_CA_1.png). What
each resume claim rests on: [docs/RESUME_CLAIMS.md](docs/RESUME_CLAIMS.md). The
write-up is issue #6.
...
- `src/m5/` - `download.py`, `verify.py`, `data.py` (one-store loaders),
  ...
  `explain.py` (SHAP), `snap.py` (SNAP lift from the raw data)
